...
@component
def gridelement(x,y, width=300, height=100):
    placeholder = f"https://placehold.co/{width}x{height}"
    grid = html.div(
        {
            "style": {
                "display": "grid",
                "gridTemplateColumns": f"repeat({x}, 1fr)",
                "gridTemplateRows": f"repeat({y}, 1fr)",
                "gap": "10px",
            }
        },
        [
            html.div(
                html.p(
                    f"Placeholder {i * x + j + 1}",
                ),
                html.img(
                    {
                        "src": placeholder,
                        "alt": f"Placeholder {i * x + j + 1}",
                    }
                )
            )
            for i in range(y)
...
@component
def Photo(path="https://placehold.co/400x200", alt="Photo"):
    return html.img(
        {
            "src": path,
            "alt": alt,
